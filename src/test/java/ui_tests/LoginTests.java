...
public class LoginTests  extends ApplicationManager {
    ...
    public void loginNegativeTest_emptyPassword(){
        User user = User.builder().username("bilbo_baggins_12345@mail.com").password("").build();
        new HomePage(getDriver()).clickBtnLoginHeader();
        LoginPage loginPage=new LoginPage(getDriver());
        loginPage.typeLoginForm(user);
    ...
    @Test
    public void loginNegativeTest_emailWOBtnYalla(){
        User user = User.builder().username("bilbo_baggins_12345@mail.com").password("Password123!").build();
        new HomePage(getDriver()).clickBtnLoginHeader();
        LoginPage loginPage=new LoginPage(getDriver());
        loginPage.typeLoginForm(user);
        Assert.assertTrue(loginPage.isYallaEnabled());
